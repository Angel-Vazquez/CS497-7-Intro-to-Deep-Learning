...
for epoch in range(num_epochs):
  ...
  # Gradient descent loop utilizing pytorch over softmax.
  loss = None

  Z = W.mm(instances) + b
  Z = Z - Z.max(0)[0]
  exp_Z = torch.exp(Z)
  sum_expZ = torch.sum(exp_Z, dim=0)
  prob = exp_Z / sum_expZ
  log_prob = torch.log(prob)
  cost = (-1.0 / numExamples) * torch.sum(groundTruth * log_prob)
  loss = cost + (0.5 * decay) * torch.sum(W**2)
...
# W (instances) * bias 
# This is because we're wrapped in torch, not numpy
pred = W.mm(instances) + b

# We get the maximum rows from all of the columns
# This returns a tuple
# 1 is a matrix, that returns max in columns
# 2 is a matrix, that returns index of where max is in each row

# a.max(0)[1] will return the index of max
pred = pred.max(0)[1]

# We want to convert back to numpy data for the next part
